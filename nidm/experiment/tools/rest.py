...
class RestParser:
    ...
    def formatDerivatives(self, derivative):
        ...
        if self.output_format == self.CLI_FORMAT:
            ...
            return tabulate(table, headers=["Derivative URI", "Measurement", "Label", "Value", "Datumtype"])
        else:
    ...
    def subjectSummaryFormat(self,result):
        if self.output_format == self.CLI_FORMAT:
            ...
            instruments = self.objectTableFormat(result['instruments'], ["Instrument UUID", "Category", "Value"])
